...
- 개수 표시와 표의 열 헤더 없이 목록을 표시하며, 편집 영역과 설정 항목은 독립된 카드로 구분합니다.
- 항목 클릭 또는 키보드 Enter로 원문만 복사합니다. 각 항목 우측의 `⋯`를 눌러야 해당 항목을 수정·삭제할 수 있습니다.
...
OS·그래픽 환경에 따라 지원되지 않을 수 있습니다. 아크릴 호출이 실패하면 일반 블러를
시도하고, 둘 다 실패하면 가독성을 위해 불투명 배경을 표시합니다. 블러 반경 자체는 Windows가 관리합니다.
실제 Windows 배경 합성은 로컬 데스크톱에서 최종 확인해야 합니다.

설정 파일은 `%APPDATA%/CopyAndPaste/appearance.json`에 저장합니다.
이전 Mica 설정은 부드러운 블러로 변환되며, 기존 패널 농도 설정 대신 새 배경 불투명도 65%로 시작합니다.
